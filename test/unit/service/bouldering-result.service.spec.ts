...
describe('Bouldering result service (unit)', () => {
  ...
  it('enforces topInTries >= zoneInTries with bulk results for a circuit', async () => {
    const boulder1 = {
      id: 0,
    } as Boulder;
    ...
    expect(boulderingResultRepositoryMock.persistLater).toHaveBeenCalledTimes(
      2, // 1 by the result creation, 1 at the end to save, they all points to the same objects so we only test the first two calls
    );
  ...
  it('enforces topInTries >= zoneInTries with bulk results for a limited contest', async () => {
    const boulder1 = {
      id: 0,
    } as Boulder;
