...
import { BoulderingGroupRankingsUpdateEventPayload } from '../../src/bouldering/group/bouldering-group.service';

describe('Group rankings websocket (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    client = socketIOClient.connect(`ws://localhost:${port}/group-rankings`);
  });
  ...
  it('emits newRankings', async () => {
    ...
    ])) as [BoulderingGroupRankingsUpdateEventPayload, unknown];

    expect(data.groupId).toEqual(round.groups[0].id);
    expect(data.rankings.type).toEqual(BoulderingRoundRankingType.CIRCUIT);
    expect(data.rankings.boulders).toEqual([boulder.id]);
